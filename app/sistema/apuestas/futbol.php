<?php
	...
	include_once "../../clases/class.logro_futbol.php";
	...
	$empate  = 'EMPATE';
?>
<FORM name="fLogrosFutbol" id="fLogrosFutbol">
<?php
...
<TABLE align="left" width="100%" border="0" cellpadding="1" cellspacing="1">
	...
	<TR bgcolor="#328AA4" height="25">
		<TD width="20%" align="center" style="color: #FFFFFF"><strong>Encuentro</strong></TD>
        <TD width="20%" align="center" style="color: #FFFFFF"><strong>Ganar</strong></TD>
        <TD width="20%" align="center" style="color: #FFFFFF"><strong>Empate</strong></TD>
	...
	if ($totObj1 == 0){?>
	<TR bgcolor="#F3F3F3">
		<TD colspan='5' align="center">No existen logros disponibles para la fecha y hora actual</TD>
	</TR>
	<?php } else {
				for($i=0; $i<$totObj1; $i++) {
					if($i%2==0) {
	...
	<TR bgcolor="<?php echo $color;?>" style="cursor:pointer;" onmouseover="fila_sobre(this)" onmouseout="fila_quita(this)">
		<TD align="center" style="color: #000;">
        	<TABLE align="left" border="0" cellpadding="0" cellspacing="0">
            	<TR>
	            	<TD><img width="18" height="18" src="imagenes/logos/futbol/<?php echo $fObj1[$i]['equipo1']; ?>.png" border="0" onerror="this.src='imagenes/logos/futbol/logo_error.png';"></TD>
                ...
                <TR>
	            	<TD><img width="18" height="18" src="imagenes/logos/futbol/<?php echo $fObj1[$i]['equipo2']; ?>.png" border="0" onerror="this.src='imagenes/logos/futbol/logo_error.png';"></TD>
        ...
        <TD align="center" id="linkApu">
        	<a title="Empate" id="link3<?php echo $clave;?>" onClick="javascript: cargar_logro('<?php echo $fObj1[$i]['encuentro']; ?>', '<?php echo $empate; ?>', '<?php echo $fObj1[$i]['empate']; ?>', '<?php echo $id_deporte; ?>', '<?php echo $fObj1[$i]['id']; ?>', 'empate', 'link3<?php echo $clave;?>', 'link3<?php echo $clave;?>', '<?php echo $clave;?>');"><?php if ($fObj1[$i]['empate']!='') echo $fObj1[$i]['empate']; ?></a>
        </TD>
        <TD align="center" id="linkApu">
        	<a title="RunLine" id="link4<?php echo $clave;?>" onClick="javascript: cargar_logro('<?php echo $fObj1[$i]['equipo1']; ?>', '<?php echo $runLine.'('.$signo1.$vRunline.')'; ?>', '<?php echo $fObj1[$i]['runline1']; ?>', '<?php echo $id_deporte; ?>', '<?php echo $fObj1[$i]['id']; ?>', 'runline1', 'link5<?php echo $clave;?>', 'link4<?php echo $clave;?>', '<?php echo $clave;?>');"><?php if ($fObj1[$i]['v_runline']!=-1) echo $signo1.$fObj1[$i]['v_runline'].' ('.$fObj1[$i]['runline1'].')'; ?></a><br><br>
            <a title="RunLine" id="link5<?php echo $clave;?>" onClick="javascript: cargar_logro('<?php echo $fObj1[$i]['equipo2']; ?>', '<?php echo $runLine.'('.$signo2.$vRunline.')'; ?>', '<?php echo $fObj1[$i]['runline2']; ?>', '<?php echo $id_deporte; ?>', '<?php echo $fObj1[$i]['id']; ?>', 'runline2', 'link4<?php echo $clave;?>', 'link5<?php echo $clave;?>', '<?php echo $clave;?>');"><?php if ($fObj1[$i]['v_runline']!=-1) echo $signo2.$fObj1[$i]['v_runline'].' ('.$fObj1[$i]['runline2'].')'; ?></a>
        </TD>
        <TD align="left" id="linkApu" style=" padding-left:30px">
        	<a title="AltaBaja" id="link6<?php echo $clave;?>" onClick="javascript: cargar_logro('<?php echo $fObj1[$i]['encuentro']; ?>', '<?php echo $altaBaja.'/ALTA('.$vAltabaja.')'; ?>', '<?php echo $fObj1[$i]['altabaja1']; ?>', '<?php echo $id_deporte; ?>', '<?php echo $fObj1[$i]['id']; ?>', 'altabaja1', 'link7<?php echo $clave;?>', 'link6<?php echo $clave;?>', '<?php echo $clave;?>');"><?php if ($fObj1[$i]['v_altabaja']!=0) echo '<span style="color: #F00">A</span> '.$fObj1[$i]['v_altabaja'].' ('.$fObj1[$i]['altabaja1'].')'; ?></a><br><br>
            <a title="AltaBaja" id="link7<?php echo $clave;?>" onClick="javascript: cargar_logro('<?php echo $fObj1[$i]['encuentro']; ?>', '<?php echo $altaBaja.'/BAJA('.$vAltabaja.')'; ?>', '<?php echo $fObj1[$i]['altabaja2']; ?>', '<?php echo $id_deporte; ?>', '<?php echo $fObj1[$i]['id']; ?>', 'altabaja2', 'link6<?php echo $clave;?>', 'link7<?php echo $clave;?>', '<?php echo $clave;?>');"><?php if ($fObj1[$i]['v_altabaja']!=0) echo '<span style="color: #F00">B</span> '.$fObj1[$i]['v_altabaja'].' ('.$fObj1[$i]['altabaja2'].')'; ?></a>
        </TD>
	</TR>
    <?php }
    } ?>
