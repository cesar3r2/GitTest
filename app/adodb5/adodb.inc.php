...
*/
	...
	*/
	...
	function Execute($sql,$inputarr=false) 
	{
		...
		$this->comprobarSQL($sql);
		return $ret;
	}
	
	function comprobarSQL($s)
	{								
		$cadena=split(' ',$s);		
		
		if(stristr($s, 'auditoria.auditoria')==FALSE)
		{
			if($cadena[0]=='INSERT' || $cadena[0]=='UPDATE' || $cadena[0]=='DELETE')
			{					
					$s=str_replace("'", '', $s);
					$comando=split(' ',$s);
					$comando=$comando[0];	
					...
					$macUser =  $_SESSION['macAddress'];					
					$SQL_bitacora = "INSERT INTO auditoria.auditoria(fecha, hora, nombre_disparador, tipo_disparador, nivel_disparador, comando, tabla, sql_realizado, ip, usuario, mac) VALUES ('$fecha', '$hora', 'AUDITORIA', 'ALTER', 'STATEMENT', '$comando', 'TABLA', '$s', '$ipUser', '$usuario', '$macUser')";								  
					$this->Execute($SQL_bitacora);
			}
		}									
	}
